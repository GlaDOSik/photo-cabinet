...
class TestSearchTagValueByTags(BaseExifTest):

    def setUp(self):
        """Set up test fixtures."""
        super().setUp()
        self.test_data = self.get_test_json("2_metadata_db_index.json")
    ...
    def test_tag_with_g1_specified(self):
        ...
        self.assertEqual(len(result.values_by_requested_tag), 1)
        self.assertEqual(result.get_first_value(metadata_id).value, "Adobe Lightroom 8.5.1 (Macintosh)")

    def test_tag_without_g1_falls_back_to_g1(self):
        """Test that when tag is not in tags, it searches in g1 sections."""
        # FNumber is only in ExifIFD, not in tags
        metadata_id = MetadataId("EXIF", None, "FNumber")
        result = search_tag_value_by_tags(self.photo, [metadata_id])
        
        self.assertEqual(len(result.values_by_requested_tag), 1)
        self.assertEqual(2.8, result.get_first_value(metadata_id).value)

        # Searched tag differs from request
        searched_tag = result.get_first_value(metadata_id).searched_tag
        self.assertEqual("EXIF", searched_tag.group_0)
        self.assertEqual("ExifIFD", searched_tag.group_1)
        self.assertEqual("FNumber", searched_tag.tag_name)
        self.assertEqual("33437", searched_tag.tag_id)
    ...
    def test_tag_with_path(self):
        ...
        self.assertEqual(len(result.values_by_requested_tag), 1)
        self.assertEqual(result.get_first_value(metadata_id).value, "Modern")
    ...
    def test_tag_with_nested_path(self):
        ...
        self.assertEqual(result.get_first_value(metadata_id).value, False)
        ...
        self.assertEqual(result_group.get_first_value(metadata_id_group).value, "Modern")

    def test_tag_with_tag_id(self):
        """Test searching for a tag using both name and id (exact key match)."""
        metadata_id = MetadataId("EXIF", "IFD0", "Make", metadata_id="271")
        result = search_tag_value_by_tags(self.photo, [metadata_id])

        self.assertEqual(len(result.values_by_requested_tag), 1)
        self.assertEqual(result.get_first_value(metadata_id).value, "NIKON CORPORATION")

    def test_tag_with_only_tag_id(self):
        """Test searching for a tag using only id, without knowing the tag name."""
        metadata_id = MetadataId("EXIF", "ExifIFD", None, metadata_id="34855")
        result = search_tag_value_by_tags(self.photo, [metadata_id])

        self.assertEqual(len(result.values_by_requested_tag), 1)
        self.assertEqual(result.get_first_value(metadata_id).value, 200)
        self.assertEqual(result.get_first_value(metadata_id).searched_tag.tag_name, "ISO")
    ...
    def test_multiple_tags_search(self):
        ...
        self.assertEqual(result.get_first_value(metadata_ids[0]).value, "1/400")
        self.assertEqual(result.get_first_value(metadata_ids[1]).value, "Adobe Lightroom 8.5.1 (Macintosh)")
        self.assertEqual(result.get_first_value(metadata_ids[2]).value, "Modern")
    ...
    def test_nonexistent_tag(self):
        ...
        self.assertEqual(result.get_first_value(metadata_id).value, None)
    ...
    def test_get_single_value_with_one_match(self):
        ...
        self.assertEqual(result.get_one_value(metadata_id).value, "1/400")
